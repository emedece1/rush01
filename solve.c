...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/17 19:24:24 by dcanari           #+#    #+#             */
/*   Updated: 2023/09/17 20:20:46 by dcanari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	solve(t_board board, t_views views, int *solved)
{
	...
	while(++i <= 4)
	{
		...
		if(solve(board_clone, views, solved) && check_viewpoints(board_clone, views))
		{
			*solved = TRUE;
...
int	check_line(t_board board, t_view_point view, int view_line, int building_viewed)
{
	...
	while(i < 4)
	{
		...
		if(tmp_building_floor == UNASSIGNED)
			return(FALSE);
		building_viewed--;
		while(i + 1 < 4 && tmp_building_floor > get_with_view(board, view, view_line, i+1))
			i++;
		i++;
